...
class HrController extends Controller
{
    ...
    public function employees()
    {
        ...
        foreach($emp as $k => $emps){
            ...
            if(count($member)>0){
            $man = Group::whereNotIn('_id', $member)->where('company_id', '=', $emps->cmp)->get();
        } else {
            $man = Group::where('company_id', '=', $emps->cmp)->get();
        }
            $emps->man = $man;   
    ...
    {
        $emp = User::orderBy('employee_id', 'DESC')->where('employee_id', '=', Auth::user()->employee_id)->first();
        $us = User::get();
        $eid = $emp->employee_id;
    ...
    public function store_employee(Request $request)
    {
    	...
    	$request->validate([
            ...
            'gender' => 'required',
            'email' => 'required|string|email|cmail|max:100|unique:users,email'.Auth::user()->hr_id,
            'alternate_email' => 'required|string|email|max:100|unique:users,alternate_email'.Auth::user()->hr_id,
    ...
    public function groups()
    {
    	...
    	foreach($grp as $k => $grps){
    		$user = User::where('_id', '=', $grps->manager)->first();
    ...
    public function store_group(Request $request)
    {
        ...
        $cmp = Company::where('_id', '=', Auth::user()->cmp)->first();
        ...
        $user = User::where('_id', '=', $request->input('manager'))->first();
        $user->group = $grp->id;
        $user->manager = 1; 
        $user->save();
    ...
    public function group_members($id)
    {
        ...
        foreach ($member as $key => $members) {
            $cmp = Company::where('_id', '=', $members->company_id)->first();
            $members->cmp = $cmp;
            $user = User::where('_id', '=', $members->employee_id)->first();
            $members->user = $user;
            $group_members = GroupMember::where('employee_id', '=', $member[$key]->employee_id)->pluck('group_id');
            if(count($group_members)>0){
            $group = Group::whereNotIn('_id', $group_members)->get();
        } else {
            $group = Group::get();
        }
        ...
        // dd($member);
        $emp = User::where('status', '=', "1")->orWhereNull('status')->whereNotIn('_id',$fcompany)->get();
        // dd($emp);
    ...
    public function documents()
    {
        ...
        foreach ($doc as $key => $docs) {
            $user = User::where('_id', '=', $docs->employee)->first();
            $docs->user = $user;
        }
    ...
    public function doc_store(Request $request)
    {
        ...
        foreach($emp as $k => $emps){
            ...
            if($request->input('employee')==array(0 => "all")){
                ...
                $employee = User::where('_id', '=', $emps)->first();
                $send_email = $employee->email;
        ...
        if ($request->hasFile('document')) {
                ...
                $ind = Document::where('_id','=',$doc->id)->first();
    ...
    public function slip()
    {
        ...
        $company = Company::where('_id', '=', Auth::user()->cmp)->first();
    ...
    public function awards()
    {
        ...
        foreach ($awd as $key => $awds) {
            $user = User::where('_id', '=', $awds->employee)->first();
            $awds->user = $user;
        }
    ...
    public function award_store(Request $request)
    {
        ...
        // $doc->employee = $request->input('employee');
            ...
            $image->move($destinationPath, $input['imagename']);
            $ind = Award::where('_id','=',$doc->id)->first();
    ...
    public function leave_request()
    {
        ...
        foreach($fleave as $k => $leaves){
            $user = User::where('_id', '=', $fleave[$k]->user_id)->first();
            $leaves->user = $user;
         }
    ...
    public function leave_accept($id)
    {
        ...
        $user = User::where('_id', '=', $leave->user_id)->first();
    ...
    public function profile(Request $request, $eid)
    {
        ...
        $company = Company::where('_id', '=', $user->cmp)->first();
        ...
        foreach ($manager as $key => $managers) {
            $us = User::where('_id', '=', $managers->employee_id)->first();
            $managers->us = $us;
        }
        $g = GroupMember::where('employee_id', '=', $user->id)->where('manager', '<>', 1)->first();
        if(isset($g)){
            $grp = Group::where('_id', '=', $g->group_id)->first();
        } else {
            $grp = "";
        }
    ...
    public function salary_slip($eid)
    {
        ...
        if($sal<1){
            ...
            $salary = Salary::where('employee', '=', $employee->employee_id)->where('month', '=', date('m'))->first();
            $salary->employee = $employee->employee_id;
            $salary->working_days = 30;
            $salary->worked_days = $worked;
            $salary->leave_taken = $days;
    ...
    public function salary_cron()
    {
        ...
        $current = date('F');
        $empl = User::where('_id', '=', $employee[$k]->id)->first();
        $month = Carbon::createFromFormat('F-d', "$current-1")->addMonth()->format('F');
        $mon = Carbon::createFromFormat('F-d', "$current-1")->addMonth()->format('M Y');
